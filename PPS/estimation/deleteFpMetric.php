<?php
include("config.php"); 
$query2 = "SELECT mID FROM fp_metric WHERE fpID = '".$_POST["Metric_id"]."'"; 
$metricID = mysqli_query($connect, $query2); 

$row = mysqli_fetch_array($metricID);


	$query = "DELETE FROM fp_metric WHERE fpID = '".$_POST["Metric_id"]."'";  
    ...
    <thead>
		...
		$result = mysqli_query($connect, $query);
                  ...
                  <tr>   
                   ...
                   <td><input type="button" name="view" value="view" id="<?php echo $row["fpID"]; ?>" class="btn btn-warning btn-xs view_data" />
                     
                     <input type="button" name="delete" value="delete" id="<?php echo $row["fpID"]; ?>" class="btn btn-danger btn-xs delete_fpdata" /></td>
