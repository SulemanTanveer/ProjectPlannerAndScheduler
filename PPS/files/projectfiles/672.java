...
/**
 *
 * This is a typical input buffer. The buffer will have two pointers:
 * forward and lex-pointer.
 * 
 * The current string for the DFA simulator is characters between 
 * lex to forward pointer.
 * 
 * In case both lex and forward will be pointing to a same location and DFA simulator
 * returns false then an error message will be generated.
 * 
 * In case DFA simulator return true then the forward pointer is incremented
 * one char at a time, until the DFA simulator returns false. After that we will decrement
 * forward point and extract the token and write it in files.
 * 
...
public class InputBuffer {
    ...
    /**
     * lexeme pointer
     * the value of lpoint may be negative. The non-positive value
     * points to the cache that contains a partial token from the 
     * previous buffer
     */
    int lpoint = 0;
    /**
        ...
        if (fillIndex < SIZE_OF_BUFFER) {
            if (dataFromSource == COMMENT_START) {
                if (lastCharWasASlash) {
                     ...
                     */
                    insideComment = true;
                    fillIndex--; //to overwrite single slash already in the buffer
                }
            ...
            /**
             * when the buffer is full then use it
             * to find tokens.
             */
            //printBuffer();
    ...
    private void printError() {
        System.err.println("Invalid character '" + buffer[fpoint] + "' at line number =" + currentlineNumber);
    }
