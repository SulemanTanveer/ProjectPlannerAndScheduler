        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
				   ...
				   $query = "SELECT projId FROM task WHERE taskId='".$_GET["taskId"]."'"; 
					...
					$row5 = mysqli_fetch_array($result); 
						...
						<ol class="breadcrumb">
                            ...
                            <li  >
                                 <i class="fa fa-pencil"></i> <a href="http://localhost/PPS/Project%20Manager/task.php?projId=<?php echo $row5['projId'];?>">Tasks</a>
                            </li>
							 <li class="active">
                ...
                <?php
											   ...
											   $query = "SELECT * FROM subtask WHERE taskId='".$_GET["taskId"]."' AND subtaskStatus ='Completed'"; 
											   $result = mysqli_query($connect, $query);
														
										       $count2 = mysqli_num_rows($result);
										      $query = "SELECT * FROM subtask WHERE taskId='".$_GET["taskId"]."' AND subtaskStatus ='In-Progess'"; 
											   
											  
											   $result = mysqli_query($connect, $query);
														
										       $count3 = mysqli_num_rows($result);
												$query = "SELECT * FROM subtask WHERE taskId='".$_GET["taskId"]."' AND subtaskStatus ='Not Started'"; 
...
<!-- /.row -->	  <input type='hidden' id='proj_status' value="
						<?php
						$select_query2="SELECT* from task WHERE taskId = '".$_GET["taskId"]."'";
						 $result2 = mysqli_query($connect, $select_query2);
                          $row2 = mysqli_fetch_array($result2); 
						$select_query24="SELECT* from project WHERE projId = '".$row2["projId"]."'";
						 $result24 = mysqli_query($connect, $select_query24);
                          $row24 = mysqli_fetch_array($result24); 
						 if($row24["projStatus"]=='Completed'){
							 echo '1';
						 } 
						 else{
							 echo '0';
						...
						$select_query2="SELECT* from task WHERE taskId = '".$_GET["taskId"]."'";
						 $result2 = mysqli_query($connect, $select_query2);
                          $row2 = mysqli_fetch_array($result2); 
						 $select_query23="SELECT* from subtask WHERE taskId = '".$_GET["taskId"]."'";
						 ...
						 else  if($row2["taskStatus"]=='In-Progress' && $count==0){
							 echo '2';
					  ...
					  <div class="row">
						...
						<div class="col-lg-12">
						    ...
						    <button type="button"   id="add"  class="btn btn-success"><i class="glyphicon glyphicon-plus-sign"></i> Create SubTask </button>
						</div> 
						  </form>
						  <script>
						 $('#add').click(function(){
 if($.trim($('#proj_status').val())=='1'){
...
else  if($.trim($('#task_status').val())=='1'){
	alert('Task is already completed\nCreate another task and create its subtasks..!');
	
	
}else  if($.trim($('#task_status').val())=='2'){
	alert('Task is in progress\nSubtasks can not be created..!');
    ...
    <thead>
        ...
        <tbody>
		 <?php
		include("config.php");
		 $select_query2 = "SELECT * FROM subtask WHERE taskId = '".$_GET["taskId"]."'";  
     $result2 = mysqli_query($connect, $select_query2);
     while($row2 = mysqli_fetch_array($result2))  {
    $task=$row2["subtaskId"];
   $proj=$_GET["taskId"];
   ...
   $id1= $task.$v.$proj;
                     ...
                     ?>
                  <tr>   <td><center><?php echo $row2["subtaskId"]; ?>  </center></td>
                      <td><center><?php echo $row2["subtaskName"]; ?>  </center></td>
					  <td><center><?php echo $row2["startDate"]; ?>  </center></td>
       
					  <td ><center><?php 
					 if($row2["subtaskStatus"]=='Completed')
					 {
					 echo '<p  style="background-color: #00D211; color:white">'.$row2["subtaskStatus"].'</p>'; 
					 }
					 else  if($row2["subtaskStatus"]=='In-Progress'){
						 echo '<p  style="background-color: #FDC305; color:white">'.$row2["subtaskStatus"].'</p>'; 
					 }
					 else if($row2["subtaskStatus"]=='Not Started'){
						 echo '<p  style="background-color: #FE0000; color:white">'.$row2["subtaskStatus"].'</p>'; 
					 }
					 else if($row2["subtaskStatus"]=='Pending'){
						 echo '<p  style="background-color: #428bca; color:white">'.$row2["subtaskStatus"].'</p>'; 
					 }
					 ?> </center></td>
                     <td><center><?php echo $row2["deadline"]; ?>  </center></td>
				<td><center>
				    <button type="button" name="file" value="files" id="<?php echo $row2["subtaskId"]; ?>" class="btn btn-success btn-xs file_data3" ><i  class="fa fa-paperclip"></i> Files</button> 
                    <button type="button" name="view" value="view" id="<?php echo $row2["subtaskId"]; ?>" class="btn btn-warning btn-xs view_dataaa" ><i  class="fa fa-search-plus"></i> View</button>
					 ...
					 <button type="button" name="edit" value="edit" id="<?php echo $id1; ?>" class="btn btn-info btn-xs edit_dataaa"  > <i class="fa fa-pencil-square-o" ></i> Edit</button> 
                   
                    
				 
					 <button type="button" name="delete" value="delete" id="<?php echo $id1; ?>" class="btn btn-danger btn-xs delete_dataaa" > 
...
<input type="hidden" name="task_startdate" id="task_startdate" value="<?php $sql2="SELECT * FROM task WHERE taskId = '". $_GET["taskId"]."'";$result2 = mysqli_query($connect, $sql2);$row = mysqli_fetch_array($result2);echo $row["startDate"];?>" class="form-control">
    ...
    <input type="hidden" name="tasks_count" id="tasks_count" value="<?php
	
	$sql2="SELECT * FROM taskassignment WHERE taskId = '". $_GET["taskId"]."'";
	$result2 = mysqli_query($connect, $sql2);
	$count = mysqli_num_rows($result2);
	echo $count;
	
	?>" class="form-control">
       <input type="hidden" name="subtasks_count" id="subtasks_count" value="<?php
	
	$sql2="SELECT * FROM subtask WHERE taskId = '". $_GET["taskId"]."' and empId!='0'";
	$result2 = mysqli_query($connect, $sql2);
	$count = mysqli_num_rows($result2);
	echo $count;
...
<input type="date" name="deadline" id="deadline" class="form-control" required>
			  ...
			  <label id='wh'hidden>Subtask Hours</label>
			  
    
    
   <input type="hidden" id="subtaskHours"name="subtaskHours" class="form-control" value="1" min="1" >
...
$id='';
 ...
 $query = "SELECT* FROM taskassignment WHERE taskId = '".$_GET["taskId"]."'";  
  ...
  while($row = mysqli_fetch_array($result))
     ...
     $output .=  "<tr><td><input  type = 'checkbox' class='check_perm' name = 'empList' value = '{$row2['empId']}' /></td>".'<td><img style=" border-radius: 50%;" src="' .$row2['image'] .'" width="40px" height="40px"  />' .$row2['empName'] .'</td>'.
