...
if($projId=='Un Assign'){
	$query = " INSERT INTO team(teamName,teamStatus,createdBy )  VALUES ('$teamName', '$teamStatus','$emp_id')";

	 mysqli_query($connect, $query);
		
	 
	 $query2 = "SELECT teamId FROM team ORDER BY teamId DESC LIMIT 1";
	...
	$query = " INSERT INTO team(teamName,teamStatus,createdBy )  VALUES ('$teamName', 'Active','$emp_id')";
	mysqli_query($connect, $query);
	$query2 = "SELECT teamId FROM team ORDER BY teamId DESC LIMIT 1";
	$result = mysqli_query($connect, $query2);
	 $row = mysqli_fetch_array($result);  
	...
	$query2 = "UPDATE project SET projStatus='Not Started',teamId='teamId' WHERE projId='$projId'";  
	 ...
	 if(!empty($_POST["empList"])){
				  ...
				  {
				 $query1 = "INSERT INTO teammember(teamId,empId)  VALUES ('$teamId', '$empId')";
			  ...
			  $output .= '<form>               
                    ...
                    <div class="col-lg-12">
                        <div class="alert alert-success alert-dismissable">
                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                            <i class="fa fa-check"></i>  <strong>Team Created</strong> 
							</div>
                    </div>
                </div><center>';
